...
func loadServiceConfig(ctx context.Context, errorService errors.Service) (*ServiceConfig, string, errors.Error) {
	config := &ServiceConfig{}
	loaded := false
	if boot.IsDevModule() && *configPath == "./config.yml" {
		err := bootutils.LoadConfig(ctx, "./config-dev.yml", config, errorService)
		if err == nil {
	...
	if !loaded {
		err := bootutils.LoadConfig(ctx, *configPath, config, errorService)
		if err != nil {
			return nil, "", err
		}
	}
