...
func Resolver(ctx context.Context, serviceInfo configuration.ServiceInfo, defaultSrvAddr ...string) errors.Error {
	rb, err := newResolver(ctx, serviceInfo, defaultSrvAddr...)
	if err != nil {
		return err
	}
...
func newResolver(ctx context.Context, serviceInfo configuration.ServiceInfo, defaultSrvAddr ...string) (resolver.Builder, errors.Error) {
...
func (impl *etcdResolverBuilder) Scheme() string {
	return impl.serviceInfo.Scheme
}
...
func (r *etcdResolver) initServerAddr() []resolver.Address {
	...
	if err != nil {
		r.logger.Error("etcd获取服务节点信息失败:%s", zap.Any("cause", err))
	} else {
		...
		func() {
			...
			case n, ok := <-rch:
				...
				for _, ev := range n.Events {
					...
					case mvccpb.DELETE:
						r.logger.Warn("节点丢失", zap.String("nodeAddr", addr), zap.String("rpcService", r.ServerName))
...
func (r *etcdResolver) getServiceAddr(kv *mvccpb.KeyValue) *resolver.Address {
	...
	if err != nil {
		r.logger.Error("注册信息反序列化失败", zap.Error(err), zap.String("body", string(kv.Value)))
		return nil
	}
